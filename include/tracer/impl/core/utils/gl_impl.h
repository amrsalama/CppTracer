...
void DrawRectangle(int x, int y, int width, int height, const T& data,
  ...
  // Draw rectangle.
  glColor4f(background_color.get_red(), background_color.get_green(),
  ...
  // Draw data.
  // The font used is GLUT_BITMAP_9_BY_15,
  // in which each character takes 9x15.
  glColor4f(font_color.get_red(), font_color.get_green(),
            font_color.get_blue(), font_color.get_alpha());
